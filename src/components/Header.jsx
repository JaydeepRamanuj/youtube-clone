...
function Header() {
  ...
  return (
    <>
      <div className="text-white flex p-2 items-center justify-between px-4">
        ...
        <div className="flex">
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault();
              if (value != "") {
                setToolVal((prev) => ({ ...prev, searchKey: value }));
          ...
          >
            <div className="search-bar border border-white/20 rounded-full flex relative">
              <input
                ...
                value={value}
                placeholder="Search"
                className="bg-transparent w-[450px] p-1.5 px-3 rounded-full transition-all rounded-r-none peer focus:pl-8 outline-none focus:outline-blue-500 outline-offset-0"
                onChange={(e) => {
